...
class Course:
    ...
    def _get_dom_from_webpage(self):
        ...
        course_url = "http://unt.catalog.acalog.com/preview_course.php?catoid=" + str(self.catoid) + "&coid=" + str(self.coid)
        page = urllib2.urlopen(course_url)
        return BeautifulSoup(page, 'html.parser')  # This will allow us to use the webpage as an object
        ...
        for link in links:
            if "Prerequisite(s):" in link.previous_sibling:
                if link.previous_sibling == "Prerequisite(s): ":
                    ...
                    self.add_course_to_list(self.find_catoid_from_href(link['href']), self.find_coid_from_href(link['href']), courses_that_are_pre_reqs)
                    ...
                    while True:
                        if nextelementtocheck.name == u'br':
                            ...
                            self.add_course_to_list(self.find_catoid_from_href(nextelementtocheck['href']), self.find_coid_from_href(nextelementtocheck['href']), courses_that_are_pre_reqs)
    ...
    def printinfo(self, tabs=0, show_prereqs=False,recursive_show_prereqs = False):
        ...
        if len(self.prereqs) > 0 and show_prereqs:
            ...
            for course in self.prereqs:
                # print course
                course.printinfo(tabs + 1, show_prereqs=recursive_show_prereqs, recursive_show_prereqs=recursive_show_prereqs)
        # print "\n"
    ...
    def find_coid_from_prefix_and_number(self, catoid, prefix, number):
        '''search_url = "http://catalog.unt.edu/content.php?filter[27]=" + str(prefix) + "&filter[29]=" + str(
            ...
            '''
        search_url = "http://catalog.unt.edu/search_advanced.php?cur_cat_oid="+str(catoid)+"&filter[keyword]="+str(prefix)+str("+")+str(number)+"&filter[exact_match]=1&filter[3]=1"
        # print search_url
    ...
    def __init__(self, catoid, coid=None, prefix = None, number = None, compound = None):
        if coid is None and compound is None:
            ...
            self.coid = self.find_coid_from_prefix_and_number(catoid, str(compound)[:4], str(compound)[4:])
        else:
            self.coid = coid
...
class FourYearPlan:
    ...
    def check_semester_for_prereqs(self, semesterIndex):
        ...
        # record the pre req courses into a dictionary with value False, meaning that it has not yet been marked as taken
        # check each semester, if it's earlier, then check each class.
        # If said class matches on in the above dictionary, change value to True
        # If there is a class that is still False at the end, return an error message saying that the class is missing a pre req
...
# #####
# Importer notes
# Test document: file:///Users/andrewsansom/Downloads/My%20Audit%20-%20Audit%20Results%20Tab.html
